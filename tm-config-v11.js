...
T.CFG={metered:'tommart-radio.metered.live',region:'europe',reconnect:2200};
T.TRACKS=[
...
T.titleFor=st=>{if(!st)return{title:'TomMart',sub:'Radio'};if(st.segment==='track'){const x=T.TRACKS[st.track]||T.TRACKS[0];return{title:x.title,sub:'Музыка • TomMart Radio'}}if(st.segment==='radioJingle')return{title:T.RADIO_JINGLE.title,sub:'Фирменный джингл TomMart'};if(st.segment==='adJingle')return{title:T.AD_JINGLE.title,sub:'Сейчас будет реклама'};if(st.segment==='ad'){const a=T.ADS[st.ad]||T.ADS[0];return{title:a.title,sub:'Рекламный блок'}}return{title:'TomMart',sub:'Radio'}};
...
T.adAfter=i=>[1,4].includes(((i%5)+5)%5);
T.jingleAfter=i=>[2,6].includes(((i%7)+7)%7);
T.adIndexAfter=i=>{let c=0;for(let k=0;k<=i;k++)if(T.adAfter(k))c++;return Math.max(0,c-1)%T.ADS.length};
T.sequenceFrom=anchor=>{const q=[];for(let n=0;n<T.TRACKS.length;n++){const i=(anchor+n)%T.TRACKS.length,next=(i+1)%T.TRACKS.length,doAd=T.adAfter(i),doJ=T.jingleAfter(i),ad=T.adIndexAfter(i);q.push({segment:'track',track:i,sourceTrack:i,dur:T.TRACK_DURS[i]});if(doAd){q.push({segment:'adJingle',ad,sourceTrack:i,afterJingle:doJ,nextTrack:next,dur:T.AD_JINGLE_DUR},{segment:'ad',ad,sourceTrack:i,afterJingle:doJ,nextTrack:next,dur:T.AD_DURS[ad]})}if(doJ)q.push({segment:'radioJingle',sourceTrack:i,nextTrack:next,dur:T.RADIO_JINGLE_DUR})}return q};
