...
T.$('leftLabel').textContent='СЕЙЧАС';T.$('rightLabel').textContent='СИНХРОНИЗАЦИЯ';e.left.textContent='ЗАГРУЗКА';e.right.textContent='ОБЩИЙ ЭФИР';
document.title='TomMart — Moldova Radio';

let ready=false,started=false,clockOffset=0,currentKey='',syncing=false;
...
async function syncNow(force=false,leadMs=0){
 if(!ready||!started||syncing)return;
 syncing=true;
 try{
  const st=T.timelineState(0,T.GLOBAL_ANCHOR,radioNow()+leadMs);
  ...
  if(changed){
   ...
   e.music.src=src;e.music.load();await waitMeta(e.music);
   ...
   if(force||ad>.38){try{e.music.currentTime=wanted;e.music.playbackRate=1}catch(_){}
   }else if(ad>.08){try{e.music.playbackRate=diff>0?0.97:1.03;setTimeout(()=>{try{e.music.playbackRate=1}catch(_){}},850)}catch(_){}}
...
async function start(){
 if(!ready){T.setStatus('warn','Музыка ещё подготавливается…');return}
...
e.music.onended=()=>{if(!started)return;currentKey='';setTimeout(()=>syncNow(true,500),40)};
e.music.onerror=()=>{if(started){currentKey='';T.setStatus('warn','Восстанавливаю музыку…');setTimeout(()=>syncNow(true,700),800)}};
e.btn.disabled=true;e.btnText.textContent='ПОДГОТАВЛИВАЮ…';e.btn.onclick=start;T.setStatus('warn','Подготавливаю музыку…');

setInterval(()=>syncNow(false),1800);
setInterval(async()=>{if(!started)return;await calibrateClock();syncNow(true)},60000);
const revive=()=>{if(!started)return;setTimeout(()=>syncNow(true),100)};
...
(async()=>{
 try{
  await Promise.all([T.loadDurations(),calibrateClock()]);
  ready=true;e.left.textContent='ГОТОВО';e.btn.disabled=false;e.btnText.textContent='СЛУШАТЬ TOMMART';T.setStatus('ok','Радио готово • нажми слушать');
 }catch(err){console.error(err);e.btn.disabled=true;e.btnText.textContent='ОШИБКА';T.setStatus('bad','Не удалось подготовить музыку')}
})();
})();