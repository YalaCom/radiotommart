...
if($('lead'))$('lead').textContent='Один общий эфир для всех устройств. Все песни перемешаны, реклама и джинглы идут по одной точной временной шкале.';
...
function buildLibrary(){
  const excluded=new Set([SPECIAL.radioJingle,SPECIAL.adJingle,...SPECIAL.ads]);
  ...
  dur(SPECIAL.radioJingle); dur(SPECIAL.adJingle);
  for(const a of ADS) dur(a.file);
}

function buildDay(startMs){
  const dayNo=Math.floor(startMs/86400000);
  const r=makeRng(hash32(dayNo^0x16a6c7d3));
  const out=[];
  let t=0,songsAd=0,songsJ=0,nextAd=r()<.5?2:3,nextJ=r()<.5?3:4;
...
async function calibrateClock(){
  ...
  for(let i=0;i<5;i++){
    try{
      const t0=Date.now();
      const r=await fetch('v16.html?tmclock='+t0+'-'+i,{method:'HEAD',cache:'no-store'});
...
async function loadManifest(){
  const r=await fetch('durations-v15.json?v=20260811-16',{cache:'no-store'});
...
async function boot(){
  ...
  try{
    ...
    setStatus('ok','Радио готово • '+LIB.length+' песен в общем эфире');
  }catch(e){
    console.error(e);
